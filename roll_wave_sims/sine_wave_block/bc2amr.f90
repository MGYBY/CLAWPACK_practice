...
subroutine bc2amr(val,aux,nrow,ncol,meqn,naux, hx, hy, level, time,   &
    ...
    real(kind=8) :: dist_period, epsilon, hn, un, pi_val, grav
    dist_period = 0.933d0
    epsilon = 0.05d0
    hn = 0.00798d0
    un = 1.03774d0
    ...
    if (xlo_patch < xlower-hxmarg) then
        ...
        select case(mthbc(1))
            case(0) ! User defined boundary condition
                ! Inflow boundary condition (wave maker)
                do j = 1, ncol
                    if (nxl >= 1) then
                        ! First ghost cell
                        val(1,nxl,j) = hn*(1+epsilon*sin(2*pi_val*(time+0.5d0*hx/sqrt(grav*hn))/dist_period))
                        val(2,nxl,j) = un*hn
                        val(3,nxl,j) = 0.0d0
                    end if
                    if (nxl == 2) then
                        ! second ghost cell:
                        val(1,nxl,j) = hn*(1+epsilon*sin(2*pi_val*(time+1.5d0*hx/sqrt(grav*hn))/dist_period))
                        val(2,nxl,j) = un*hn
                        val(3,nxl,j) = 0.0d0
                    end if
                end do
