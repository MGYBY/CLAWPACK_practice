...
normal_depth = 0.005333999999
normal_velocity = 1.28099972459
dist_amp = 0.20 # for two kinds of stability problems
wave_length = 0.694454985
sim_time = 170.0
output_interval = 1.0 # in seconds
output_flag = 1.0 #avoid ouput overwrite
channel_slope = 0.1192
gravity_val = 9.81
cf = gravity_val * channel_slope * 2 * normal_depth / (normal_velocity**2)
froude_num = 5.60

#time_integration = "instant"

def bathymetry(x):
    bath = channel_slope*(1.0 + dist_amp * np.sin(2.0 * np.pi * x/wave_length)) 
    return bath

def incoming_sin(state,dim,t,qbc,auxbc,num_ghost):
    """
    Incoming sin wave at left boundary.
    """
    dx = state.grid.delta[0]
    physical_time = state.t

    if (physical_time<wave_length/2.0):
        for i in range(num_ghost):
            qbc[0,i] = normal_depth*(1.0+dist_amp*np.sin(2*np.pi*(t+dx*(num_ghost-i)/np.sqrt(gravity_val*normal_depth))/wave_length))
            qbc[1,i] = froude_num*np.sqrt(gravity_val*((normal_depth*(1.0+dist_amp*np.sin(2*np.pi*(t+dx*(num_ghost-i)/np.sqrt(gravity_val*normal_depth))/wave_length)))**3.0))
        
    else:
        for i in range(num_ghost):
            qbc[0,i] = normal_depth
            qbc[1,i] = normal_velocity*normal_depth
    ...
    if t%output_interval<dt:
        ...
        if (not os.path.exists('./%s' % file_name)):
            with open(file_name, 'w') as f:
                ...
                writer.writerows(zip(np.transpose(X),np.transpose(height),np.transpose(mom)))
