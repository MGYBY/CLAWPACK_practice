...
#--------------------------
    ...
    # #-----------------------------------------
    # # Figure for 1-d depth
    # #-----------------------------------------
    plotfigure = plotdata.new_plotfigure(name='Water depth', figno=6)
    
    # Set up for axes in this figure:
    plotaxes = plotfigure.new_plotaxes()
    plotaxes.xlimits = 'auto'
    plotaxes.ylimits = [0, 4.50*hn]
    plotaxes.title = '1-d Water depth'
    def depth_slice(current_data):
        q = current_data.q
        h = q[0, :, 1]
        return h   
    # Set up for item on these axes:
    plotitem = plotaxes.new_plotitem(plot_type='1d_plot')
    plotitem.plot_var = depth_slice
