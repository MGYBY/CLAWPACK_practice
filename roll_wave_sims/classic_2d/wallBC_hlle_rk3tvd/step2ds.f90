...
!     ==========================================================
!     modified version of the classic2 standard library
...
!     # See the flux2 documentation for more information.
    ...
    1-num_ghost:my+num_ghost)
    double precision :: qmed1(num_eqn, 1-num_ghost:mx+num_ghost, &
    1-num_ghost:my+num_ghost)
    !double precision :: qmed2(num_eqn, 1-num_ghost:mx+num_ghost, &
    !1-num_ghost:my+num_ghost)
    ...
    double precision :: dtdx1dd(1-num_ghost:maxm+num_ghost) ! variable time step in RK3
    double precision :: dtdy1dd(1-num_ghost:maxm+num_ghost) !  variable time step in RK3
    integer :: method(7),mthlim(num_waves)
    logical ::          use_fwave
    double precision :: work(mwork)
    external :: rpn2,rpt2

    real(kind=8) :: dtt ! variable time step in RK3
...
!     # flux2 routine.  Find starting index of each piece:
    ...
    !     # three stages in x-sweeps
    !     cases for capacity function NOT implemented
        ...
        !        # in order to save storage.)
            ...
            !            # no capa array.  Standard flux differencing:
                do i = 1, mx
                    do m = 1, num_eqn
                        qmed1(m,i,j) = qnew(m,i,j) + qadd(m,i) &
        ...
        dtt = dt*(1.0/4.0)
        do i = 1-num_ghost, mx+num_ghost
            dtdx1dd(i) = dtdx1d(i)*(1.0d0/4.0d0)
        end do
        call bc2(meqn,mbc,mx,my,xlower,ylower,dx,dy,qmed1,maux,aux,(told+dt),dt,mthbc)
        do j = 1-num_ghost,my+num_ghost
            !        # copy data along a slice into 1d arrays:
            ...
            !        # compute modifications fadd and gadd to fluxes along this slice:
                call flux2(1,maxm,num_eqn,num_waves,num_aux,num_ghost,mx, &
                q1d,dtdx1dd,aux1,aux2,aux3,method,mthlim, &
                qadd,fadd,gadd,cfl1d, &
                work(i0wave),work(i0s),work(i0amdq),work(i0apdq), &
                work(i0cqxx),work(i0bmadq),work(i0bpadq),rpn2,rpt2, &
                use_fwave)
            ...
            !        # in order to save storage.)
                ...
                !            # no capa array.  Standard flux differencing:
                    do i = 1, mx
                        do m = 1, num_eqn
                            qmed1(m,i,j) = (3.0d0/4.0d0) * qnew(m,i,j)+ (1.0d0/4.0d0) * qmed1(m,i,j) + qadd(m,i) &
                                - dtdx* (1.0d0/4.0d0) * (fadd(m,i+1) - fadd(m,i))
                        end do
                    end do
        ...
        dtt = dt*(2.0/3.0)
        do i = 1-num_ghost, mx+num_ghost
            dtdx1dd(i) = dtdx1d(i)*(2.0d0/3.0d0)
        end do
        call bc2(meqn,mbc,mx,my,xlower,ylower,dx,dy,qmed1,maux,aux,(told+(1.0d0/2.0d0)*dt),dt,mthbc)
        do j = 1-num_ghost,my+num_ghost
            !        # copy data along a slice into 1d arrays:
            ...
            !        # in order to save storage.)
                ...
                !            # no capa array.  Standard flux differencing:
                    do i = 1, mx
                        do m = 1, num_eqn
                            qnew(m,i,j) = (1.0d0/3.0d0) * qnew(m,i,j)+ (2.0d0/3.0d0) * qmed1(m,i,j) + qadd(m,i) &
                                - dtdx* (2.0d0/3.0d0) * (fadd(m,i+1) - fadd(m,i))
                        end do
                    end do
    ...
    !     ==================
        ...
        !        # gadd is the modification to f-fluxes to the left and right.
            ...
            !            # no capa array.  Standard flux differencing:
                do j = 1, my
                    do m = 1, num_eqn
                        qmed1(m,i,j) = qnew(m,i,j) + qadd(m,j) &
        ...
        do i = 1-num_ghost, mx+num_ghost
            ...
            !        # compute modifications fadd and gadd to fluxes along this slice:
                call flux2(2,maxm,num_eqn,num_waves,num_aux,num_ghost,my, &
                q1d,dtdy1dd,aux1,aux2,aux3,method,mthlim, &
                qadd,fadd,gadd,cfl1d, &
                work(i0wave),work(i0s),work(i0amdq),work(i0apdq), &
                work(i0cqxx),work(i0bmadq),work(i0bpadq),rpn2,rpt2, &
                use_fwave)
